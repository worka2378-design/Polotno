    ...
    return (
      ...
      >
        ...
        <div className="flex-1 overflow-hidden mr-1 flex items-center gap-1.5 min-w-0">
          {layer.item.pinned && (
            <Pin className="w-4 h-4 text-stone-700 fill-stone-700 shrink-0" title="Закріплено" />
          )}
          {isLocked && (
            <Lock className="w-4 h-4 text-stone-700 shrink-0" title="Заблоковано" />
          )}
          {isHidden && (
            <EyeOff className="w-4 h-4 text-stone-700 shrink-0" title="Сховано" />
          )}
